...
class ImageSequence(keras.utils.Sequence):
    def __init__(self, directory_name, batch_size, captcha_length, captcha_symbols, captcha_width, captcha_height):
        ...
        self.backup_files = copy.deepcopy(self.files)
        print(f'file {hex(id(self.files))} backup {hex(id(self.backup_files))}')
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            if len(self.files.keys()) < 1:
                print("testing if loop", self.backup_files)
                self.files = copy.deepcopy(self.backup_files)
            ...
            dilate = cv2.dilate(data, (1, 3), iterations=3)
            #rgb_data = cv2.cvtColor(raw_data, cv2.COLOR_BGR2RGB)
            #blur = cv2.medianBlur(raw_data, 3)
            #edges = cv2.Canny(blur, 170, 270)
            processed_data = numpy.array(dilate)[..., numpy.newaxis] / 255.0
            #processed_data = numpy.array(rgb_data) / 255.0
            #print([i for i in processed_data])
            X[i] = processed_data
